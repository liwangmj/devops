...
yaourt -S base-devel crosstool-ng ptxdist perl lua51 lua52 luajit python2 python3 python-pip python2-pip jdk8-openjdk mysql-utilities subversion git mercurial cmake maven gradle valgrind gdb cppcheck vim lsof nload tcpdump doxygen graphviz docker docker-compose lrzsz tmux calc create_ap create_ap-gui dos2unix zip unrar wget gnu-netcat adobe-source-code-pro-fonts wqy-microhei fcitx-configtool kcm-fcitx fcitx-im fcitx unzip unrar curl epdfview

# 安装扩展应用
# yaourt -S fcitx-sogoupinyin wine winetricks electronic-wechat gitter nutstore everpad youdao-dict vlc steam deepin-screenshot pepper-flash diffmerge wps-office mockingbot drawio-desktop-bin freeplane plantuml pencil google-webdesigner gimp inkscape blender davinci-resolve freecad devdocs-desktop zeal appimage-git wxhexeditor gvim visual-studio-code-bin eclipse-jee qtcreator android-studio mysql-workbench dbeaver jlink netbeans intellij-idea-ultimate-edition pycharm-professional webstorm clion gitkraken kdesvn soapui jmeter chromedriver chromium firefox freerdp remmina wireshark-qt shadowsocks-qt5 shadowsocks packetsender filezilla thunderbird
