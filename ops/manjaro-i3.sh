...
yaourt -S docker docker-compose cmake git svn valgrind gdb luajit python3 python-pip python2-pip adobe-source-code-pro-fonts wqy-microhei xmind eclipse-jee atom qtcreator kdesvn mysql-workbench mysql-utilities freerdp remmina gnu-netcat wireshark-qt nload tcpdump chromium firefox filezilla dia vlc thunderbird fcitx-configtool kcm-fcitx fcitx-im lrzsz tmux leafpad calc wine winetricks create_ap create_ap-gui meld doxygen graphviz shadowsocks-qt5 shadowsocks dos2unix zip unrar wget
